...
class CYData:
    def __init__(self, polytope, polytope_index, triangulation, cy):
        self.p, self.p_i = polytope, polytope_index # index : [i,j,k], ith polytope, jth triangulation out of k
        self.t = triangulation
        self.cy = cy

        self.h_l, self.scc, self.kijk = cy.h12(), cy.second_chern_class(in_basis = True), cy.intersection_numbers(in_basis = True)
...
def _identify_nilpotent(cy, max_deg = 3):
...
def _diffeo_class(polytopes, polytope_tqdm):
    diffeo = {}

    for i, p in tqdm(enumerate(polytopes), disable = ~polytope_tqdm):
        ...
        for j, t in enumerate(polytope_frst):
            ...
            if wall_data not in diffeo:
                ...
                obj = CYData(p, p_i, t, cy)

                obj.assign_nop_curves(_identify_nilpotent(cy))

                diffeo[wall_data] = obj
...
def _birational_flop_wall_data(wall_data, ray_gv):
    ...
    print("wall_data : ", wall_data)
    print("ray_gv : ", ray_gv)
    print("flop_wall_data : ", flop_wall_data, "\n")

    return flop_wall_data

def _birational_class(diffeo):
    for wall_data, cydata in diffeo.items():
        ray_gv_list = cydata.nop_ray_gv_list
        for ray_gv in ray_gv_list:
            flop_wall_data = _birational_flop_wall_data(wall_data, ray_gv)
            if flop_wall_data in diffeo: # toric found
                cydata.assign_birational_duplicate(flop_wall_data, ray_gv)
    return diffeo

def diffeo_class(h_s, polytope_no_max = 10, polytope_tqdm = False, mode = "load", nontrivial_ntfe_frsts = False):
    if mode == "save" or mode == "run":
        polytopes = fetch_polytopes(h11 = h_s, lattice = "N", limit = polytope_no_max)
        diffeo = _diffeo_class(polytopes, polytope_tqdm)
        diffeo = _birational_class(diffeo)
