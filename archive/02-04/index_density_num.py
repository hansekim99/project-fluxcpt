...
class CalabiYau:
    def __init__(self, h21, arrayK, kahler_rays = np.array([1.]),
        ...
        self.msno = int(1e4)                          # size of paralellised calculations
        self.mrno = int(moduli_sample_no / self.msno) # size of for loop

        self.rng = np.random.default_rng()

    def _set_flux_mc_paras(self, quanta_sample_no = int(1e3), quanta_range = 5):
        self.qr = quanta_range

        self.qsno = int(1e2)                          # size of paralellised calculations
    ...
    def _quanta_uniform_sample(self, sample_mode, u = None):
        if sample_mode == "uniform":
            ...
            dim = 2 * (self.n + 1)
            u = self.rng.random((self.qsno, dim))

            erf_qr = erf(self.qr)
            t = erfinv((2.0 * u - 1.0) * erf_qr)
    ...
    def _ms_num(self, ms):
        ...
        scalarK = -np.log(8 * polyK)

        polyK_a = 1/2 * np.einsum('abc,nb,nc->na', self.arrayK, ms, ms)
        polyK_ab = np.einsum('abc,nc->nab', self.arrayK, ms)
        
        matrG_num = -4 * (polyK_ab[:,:,:] * polyK[:,None,None] - polyK_a[:,:,None] * polyK_a[:,None,:]) / polyK[:,None,None] ** 2
        
        _, logdetG_num = np.linalg.slogdet(matrG_num)
        
        vb_num = np.linalg.cholesky(matrG_num)
    ...
    def _flux_num(self, zzs, xxs, specgeo):
        ...
        exp_imp_pdist_log = xas_sr + np.sum(zas_sr, axis=1)
    ...
    def calc_vacua_density_flux(self, specgeo, type, qrl = False, 
        ...
        for _ in tqdm(range(qrno), disable = not qrl):
            ...
            scalar = averaged_scalar * flux_volume

            index_vacua_density += scalar * pi_pref / qrno
